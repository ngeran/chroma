...
export async function downloadZipExport(scheme: ColorScheme) {
  const zip = new JSZip();
  
  // Add all export files to the zip
  for (const exporter of exporters) {
    const content = exporter.generate(scheme);
    zip.file(exporter.filename(scheme), content);
  }
  
  // Add a README file with scheme information
  const readme = `# ${scheme.name.toUpperCase()} Theme Package
...
## OLED Optimized
This theme package is optimized for OLED displays with:
- Pure black background (#000000)
- Low-luminance colors to prevent burn-in
- High contrast for readability

## Files Included
${exporters.map(e => `- **${e.filename(scheme)}** - ${e.name}`).join('\n')}

## Installation
Each file is configured for a specific application:
- \`colors.toml\` - Omarchy color scheme (place in ~/.config/omarchy/)
- \`*.theme\` - Btop system monitor theme
- \`ghostty.conf\` - Ghostty terminal configuration
- \`hyprlock.conf\` - Hyprlock screen locker
- \`neovim.lua\` - Neovim configuration
- \`waybar.css\` - Waybar status bar style
- \`walker.css\` - Walker launcher style

## Generated by ChromaVoid
OLED-optimized color scheme generator
`;
  ...
  const link = document.createElement('a');
  link.href = url;
  link.download = `${scheme.name.toLowerCase().replace(/\s+/g, '-')}-theme-pack.zip`;
  document.body.appendChild(link);
  link.click();
  document.body.removeChild(link);
  URL.revokeObjectURL(url);
